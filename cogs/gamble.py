...
class GambleListModal(discord.ui.Modal):
    def __init__(self, user_id, mode, part):
        ...
        for i in range(5):
            ...
            default_val = ""
            if mode == "chance": 
                v = setup_cache[user_id]["chances"][idx-1]
                if v > 0: default_val = str(v)
            elif mode == "image":
                v = setup_cache[user_id]["prizes"][idx-1]
                if v: default_val = v
            else:
                v = setup_cache[user_id]["names"][idx-1]
                if v: default_val = v
...
class GambleMainView(discord.ui.View):
    ...
    async def check_stock(self, interaction: discord.Interaction):
        ...
        names = self.config.get("names", [f"Item {i+1}" for i in range(15)])
