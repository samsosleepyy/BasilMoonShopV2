...
class GambleSystem(commands.Cog):
    ...
    async def restock(self, interaction: discord.Interaction, message_link: str):
        ...
        try:
            ...
            return await interaction.response.send_message(MESSAGES["res_err_invalid_link"], ephemeral=True)

        data = load_data()
        if str(msg_id) not in data["gamble_configs"]:
            return await interaction.response.send_message(MESSAGES["res_err_not_found"], ephemeral=True)

        restock_cache[interaction.user.id] = {
...
# SETUP VIEWS (GAMBLE)
# =========================================
# ... (GambleSetupView1, GambleStep1Modal, GambleSetupView2, GambleListModal เหมือนเดิม ไม่ต้องแก้) ...
# เพื่อความชัวร์ ผมจะแปะส่วนที่เกี่ยวข้องกับ Flow ปกติให้ด้วย
...
class GambleSetupView2(discord.ui.View):
    ...
    async def cf_c1(self, interaction: discord.Interaction, button: discord.ui.Button):
    ...
    async def cf_c2(self, interaction: discord.Interaction, button: discord.ui.Button):
    ...
    async def cf_c3(self, interaction: discord.Interaction, button: discord.ui.Button):
    ...
    async def cf_i1(self, interaction: discord.Interaction, button: discord.ui.Button):
    ...
    async def cf_i2(self, interaction: discord.Interaction, button: discord.ui.Button):
    ...
    async def cf_i3(self, interaction: discord.Interaction, button: discord.ui.Button):
...
class GambleListModal(discord.ui.Modal):
    def __init__(self, user_id, mode, part, is_restock=False):
        title_key = "gam_modal_chance_title" if mode == "chance" else "gam_modal_img_title"
        if mode == "name": title_key = "gam_modal_name_title"
        ...
        self.inputs = []
        target_cache = restock_cache if is_restock else setup_cache
        for i in range(5):
            idx = self.start_idx + i + 1
            if mode == "chance": lbl_key, placeholder = "gam_lbl_c_prefix", "e.g. 10.5"
            elif mode == "image": lbl_key, placeholder = "gam_lbl_i_prefix", "https://..."
            else: lbl_key, placeholder = "gam_lbl_n_prefix", MESSAGES["gam_ph_name"]
            default_val = ""
            if mode == "chance": 
                v = target_cache[user_id]["chances"][idx-1]
                if v > 0: default_val = str(v)
            elif mode == "image":
                v = target_cache[user_id]["prizes"][idx-1]
                if v: default_val = v
            else:
                v = target_cache[user_id]["names"][idx-1]
                if v: default_val = v
            item = discord.ui.TextInput(label=MESSAGES[lbl_key].format(n=idx), placeholder=placeholder, required=False, default=default_val)
            self.add_item(item)
            self.inputs.append(item)
    async def on_submit(self, interaction: discord.Interaction):
        target_cache = restock_cache if self.is_restock else setup_cache
        cache = target_cache[self.user_id]
        if self.mode == "chance":
            try:
            ...
            for i, item in enumerate(self.inputs):
                val = item.value.strip()
                if val: cache["names"][self.start_idx + i] = val
...
class GambleSetupView3(discord.ui.View):
    ...
    @discord.ui.select(placeholder=MESSAGES["gam_setup_3_select_mode"], options=[
        discord.SelectOption(label=MESSAGES["gam_mode_unlimited"], value="unlimited", emoji="🔄"),
        discord.SelectOption(label=MESSAGES["gam_mode_limited"], value="limited", emoji="⛔")
    ])
...
class GambleSetupView4(discord.ui.View):
    def __init__(self, user_id):
        super().__init__(timeout=None)
        self.user_id = user_id
    @discord.ui.button(label=MESSAGES["gam_btn_name_1"], style=discord.ButtonStyle.secondary, row=1)
    async def config_name_1(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(GambleListModal(self.user_id, "name", 1))
    @discord.ui.button(label=MESSAGES["gam_btn_name_2"], style=discord.ButtonStyle.secondary, row=1)
    async def config_name_2(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(GambleListModal(self.user_id, "name", 2))
    @discord.ui.button(label=MESSAGES["gam_btn_name_3"], style=discord.ButtonStyle.secondary, row=1)
    async def config_name_3(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(GambleListModal(self.user_id, "name", 3))
    @discord.ui.button(label=MESSAGES["gam_setup_2_next"], style=discord.ButtonStyle.green, row=2)
    async def go_next(self, interaction: discord.Interaction, button: discord.ui.Button):
        view = GambleSetupView5(self.user_id)
        await interaction.response.edit_message(content=MESSAGES["gam_setup_5_msg"], view=view)

class GambleSetupView5(discord.ui.View):
    def __init__(self, user_id):
        super().__init__(timeout=None)
        self.user_id = user_id
    ...
    async def finish(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.user_id: return
        if "approval_channel" not in setup_cache[self.user_id]: return await interaction.response.send_message("❌ กรุณาเลือกช่องอนุมัติก่อน", ephemeral=True)
        await interaction.response.defer(ephemeral=True)
        cache = setup_cache[self.user_id]
        ...
        data = load_data()
        save_cache = cache.copy()
        save_cache["target_channel"] = raw_channel.id
        save_cache["log_channel"] = cache["log_channel"].id
        save_cache["approval_channel"] = cache["approval_channel"].id
        data["gamble_configs"][str(msg.id)] = save_cache
        save_data(data)
...
class RestockView(discord.ui.View):
    ...
    @discord.ui.button(label=MESSAGES["gam_btn_chance_1"], style=discord.ButtonStyle.secondary, row=0)
    async def rc_c1(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(GambleListModal(self.user_id, "chance", 1, is_restock=True))
    @discord.ui.button(label=MESSAGES["gam_btn_chance_2"], style=discord.ButtonStyle.secondary, row=0)
    async def rc_c2(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(GambleListModal(self.user_id, "chance", 2, is_restock=True))
    @discord.ui.button(label=MESSAGES["gam_btn_chance_3"], style=discord.ButtonStyle.secondary, row=0)
    async def rc_c3(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(GambleListModal(self.user_id, "chance", 3, is_restock=True))

    @discord.ui.button(label=MESSAGES["gam_btn_img_1"], style=discord.ButtonStyle.secondary, row=1)
    async def rc_i1(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(GambleListModal(self.user_id, "image", 1, is_restock=True))
    @discord.ui.button(label=MESSAGES["gam_btn_img_2"], style=discord.ButtonStyle.secondary, row=1)
    async def rc_i2(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(GambleListModal(self.user_id, "image", 2, is_restock=True))
    @discord.ui.button(label=MESSAGES["gam_btn_img_3"], style=discord.ButtonStyle.secondary, row=1)
    async def rc_i3(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(GambleListModal(self.user_id, "image", 3, is_restock=True))

    @discord.ui.button(label=MESSAGES["gam_btn_name_1"], style=discord.ButtonStyle.secondary, row=2)
    async def rc_n1(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(GambleListModal(self.user_id, "name", 1, is_restock=True))
    @discord.ui.button(label=MESSAGES["gam_btn_name_2"], style=discord.ButtonStyle.secondary, row=2)
    async def rc_n2(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(GambleListModal(self.user_id, "name", 2, is_restock=True))
    @discord.ui.button(label=MESSAGES["gam_btn_name_3"], style=discord.ButtonStyle.secondary, row=2)
    async def rc_n3(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(GambleListModal(self.user_id, "name", 3, is_restock=True))

    @discord.ui.button(label="ยืนยัน Restock ✅", style=discord.ButtonStyle.success, row=3)
    async def finish_restock(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.user_id: return
        await interaction.response.defer(ephemeral=True)
        
        cache = restock_cache[self.user_id]
        msg_id = str(cache["target_msg_id"])
        chan_id = cache["target_channel_id"]
        
        data = load_data()
        if msg_id not in data["gamble_configs"]:
            return await interaction.followup.send(MESSAGES["res_err_not_found"], ephemeral=True)
        
        # Update Data
        data["gamble_configs"][msg_id]["chances"] = cache["chances"]
        data["gamble_configs"][msg_id]["prizes"] = cache["prizes"]
        data["gamble_configs"][msg_id]["names"] = cache["names"]
        
        # Reset Claims & Stats
        if "claimed_prizes" in data and msg_id in data["claimed_prizes"]:
            del data["claimed_prizes"][msg_id]
        if "gamble_stats" in data and msg_id in data["gamble_stats"]:
            del data["gamble_stats"][msg_id]
            
        save_data(data)
        
        # [NEW] Re-enable Button on Main Message
        try:
            ...
            new_config = data["gamble_configs"][msg_id]
            new_view = GambleMainView(new_config)
            await msg.edit(view=new_view)
        except: pass

        # [NEW] Log Restock
        log_id = data["gamble_configs"][msg_id].get("log_channel")
        if log_id:
            log_chan = interaction.guild.get_channel(log_id)
            if log_chan:
                embed_log = discord.Embed(title=MESSAGES["res_log_title"], description=MESSAGES["res_log_desc"].format(admin=interaction.user.mention), color=discord.Color.blue())
                items_desc = ""
                names = cache["names"]
                for i, n in enumerate(names):
                     if cache["chances"][i] > 0:
                         items_desc += f"• {n}\n"
                if items_desc: embed_log.add_field(name=MESSAGES["res_log_items"], value=items_desc[:1024])
...
class GambleMainView(discord.ui.View):
    ...
    async def check_stock(self, interaction: discord.Interaction):
        ...
        config = self.config
        if msg_id in data["gamble_configs"]: config = data["gamble_configs"][msg_id]
        
        claimed_map = data.get("claimed_prizes", {}).get(msg_id, {})
        game_mode = config.get("gacha_mode", "unlimited")
        prizes = config["prizes"]
        names = config.get("names", [f"Item {i+1}" for i in range(15)])
    ...
    async def play_gacha(self, interaction: discord.Interaction):
        ...
        cost = config["cost"]
        if points < cost: return await interaction.followup.send(MESSAGES["insufficient_points"].format(cost=cost), ephemeral=True)
        
        data["points"][user_id] = points - cost
        
        # Increment Stats
        if "gamble_stats" not in data: data["gamble_stats"] = {}
        if msg_id not in data["gamble_stats"]: data["gamble_stats"][msg_id] = {}
        user_stats = data["gamble_stats"][msg_id].get(user_id, 0) + 1
        data["gamble_stats"][msg_id][user_id] = user_stats
        save_data(data)
        ...
        # [NEW] Logic: Must clear Normal (0-9) before Special (10-14)
        # Only consider valid prizes (have URL)
        valid_normal_indices = [i for i in range(10) if prizes[i]]
        claimed_normals_count = sum(1 for i in valid_normal_indices if str(i) in claimed_map)
        
        target_range = range(15)
        if game_mode == "limited":
            if claimed_normals_count < len(valid_normal_indices):
                target_range = range(10) # Lock special
            else:
                target_range = range(15) # Unlock all
        
        valid_indices = []
        valid_weights = []
        for i in target_range:
            if chances[i] > 0 and prizes[i]:
                if game_mode == "limited" and str(i) in claimed_map: continue
        ...
        embed_anim.set_image(url=config["img_gacha"])
        ...
        if log_id:
            ...
            if log_chan:
                ...
                embed_log.set_thumbnail(url=prizes[prize_index])
                await log_chan.send(embed=embed_log)
...
# ... (TopUpTMModal, PromptPayConfirmView, AdminSlipCheckView, AdminGivePointModal, AdminDenySlipModal เหมือนเดิม) ...
# ... (ก๊อปปี้จากโค้ดเก่าได้เลยครับ เพื่อไม่ให้ข้อความยาวเกินไป) ...
